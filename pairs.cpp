...
int PairAlign::GetPairs(int na, int nb)
{
...
void PairAlign::s_OutHitPair(PairHit pp, int n, RefSeq &ref, string &os) {
    ...
    if(param.out_sam) {
        ...
        if(pp.chain^(pp.a.chr%2)){
            ...
            //flag|=0x80; //second fragment
            seg_start=pp.b.loc+1; pp_insert=-pp.insert;
            reverse(_sa._pread->seq.begin(), _sa._pread->seq.end());
            for(ii=0;ii<_sa._pread->seq.size();ii++) _sa._pread->seq[ii]=rev_char[_sa._pread->seq[ii]];
      	    reverse(_sa._pread->qual.begin(), _sa._pread->qual.end());
        }
        else {
            flag|=0x20; //reverse mate read sequence
            //flag|=0x40; //first fragment
            seg_start=pp.a.loc+1; pp_insert=pp.insert;
        }
        flag|=0x40*_sa._pread->readset;

        sprintf(_ch,"%s\t%d\t%s\t%u\t255\t%dM\t=\t%u\t%d\t%s\t%s\tNM:i:%d",
            _sa._pread->name.c_str(),flag,ref.title[pp.a.chr].name.c_str(),pp.a.loc+1,_sa._pread->seq.size(),pp.b.loc+1,pp_insert,_sa._pread->seq.c_str(),_sa._pread->qual.c_str(),pp.na);
        ...
        if(param.out_ref) {
            ...
            for(ii=0;ii<_sa._pread->seq.size()+2;ii++,ptr++) {
        ...
        if((pp.chain^(pp.a.chr%2))&&param.report_repeat_hits>1){
            reverse(_sa._pread->seq.begin(), _sa._pread->seq.end());
            for(ii=0;ii<_sa._pread->seq.size();ii++) _sa._pread->seq[ii]=rev_char[_sa._pread->seq[ii]];
  	    reverse(_sa._pread->qual.begin(), _sa._pread->qual.end());
        }
        ...
        if((!pp.chain)^(pp.b.chr%2)){
            ...
            //flag|=0x80; //second fragment
            seg_start=pp.a.loc+1; pp_insert=-pp.insert;
            reverse(_sb._pread->seq.begin(), _sb._pread->seq.end());
            for(ii=0;ii<_sb._pread->seq.size();ii++) _sb._pread->seq[ii]=rev_char[_sb._pread->seq[ii]];
      	    reverse(_sb._pread->qual.begin(), _sb._pread->qual.end());
        }
        else {
            flag|=0x20; //reverse mate read sequence
            //flag|=0x40; //first fragment
        ...
        sprintf(_ch,"%s\t%d\t%s\t%u\t255\t%dM\t=\t%u\t%d\t%s\t%s\tNM:i:%d",
            _sb._pread->name.c_str(),flag,ref.title[pp.b.chr].name.c_str(),pp.b.loc+1,_sb._pread->seq.size(),pp.a.loc+1,pp_insert,_sb._pread->seq.c_str(),_sb._pread->qual.c_str(),pp.nb);
        ...
        if(param.out_ref) {
            ...
            for(ii=0;ii<_sb._pread->seq.size()+2;ii++,ptr++) {
        ...
        if(((!pp.chain)^(pp.b.chr%2))&&param.report_repeat_hits>1){
       	    reverse(_sb._pread->seq.begin(), _sb._pread->seq.end());
            for(ii=0;ii<_sb._pread->seq.size();ii++) _sb._pread->seq[ii]=rev_char[_sb._pread->seq[ii]];
  	    reverse(_sb._pread->qual.begin(), _sb._pread->qual.end());
        }
...
void PairAlign::s_OutHitUnpair(int readinpair, int chain_a, int chain_b, int ma, int na, Hit ha, int mb, Hit hb, RefSeq &ref, string &os) {
    int ii, flag=1;
    pair<ref_loc_t,int> seg_info;
        ...
        if(ma<=0){ //QC, NM
            ...
            if(chain_a^(ha.chr%2)){
                ...
                sprintf(_ch,"%s\t%d\t%s\t%u\t255\t%dM\t*\t0\t0\t%s\t%s\tNM:i:%d",
           	    ...
           	    if(chain_b^(hb.chr%2)) flag|=0x020;
                sprintf(_ch,"%s\t%d\t%s\t%u\t255\t%dM\t%s\t%u\t0\t%s\t%s\tNM:i:%d",
                    _stmp->_pread->name.c_str(),flag,ref.title[ha.chr].name.c_str(),ha.loc+1,_stmp->_pread->seq.size(),ref.title[hb.chr].name.c_str(),hb.loc+1,_stmp->_pread->seq.c_str(),_stmp->_pread->qual.c_str(),na);
           	}
            ...
            if(param.out_ref) {
                ...
                for(ii=0;ii<_stmp->_pread->seq.size()+2;ii++,ptr++) {
