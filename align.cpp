...
int SingleAlign::TrimLowQual()
{
	...
	for(_sq=_pread->qual.rbegin(); _sq!=_pread->qual.rend(); i--,++_sq) {
		if(*_sq >read_zero_qual+param.qual_threshold) {
			...
			if(n >= 3) { //define boundary at 3'-end of >=3 continuous high-quality bps
				if(i >= param.seed_size) {
					_pread->qual.resize(i+n-1);
					_pread->seq.resize(i+n-1);
					return 1;
				}
			}
		}
...
{
   	bit32_t i,j,m,w, *ptr_chr; int h, modeindex;
   	Hit prefetch_hit;

    if(param.RRBS_flag){
    	//direct chain
    	if(param.chains||(_pread->readset<2)) {
    		...
    		if((m=ref.index[_seed].n1)>0){ 
        		...
        		h=param.profile[modeindex][0].a+seed_start_offset;
        		for(j=0; j!=m; j++) {
    	    		_hit=_refloc[j];
    	...
    	//cout<<"reverse chain, mode "<<mode<<endl;
    		...
    		//cout<<"mode:"<<mode<<" m="<<ref.index[_seed].n1<<" seed:"<<param.StrSeed(param.map3to4(_seed),param.seed_size)<<endl;
    		if((m=ref.index[_seed].n1)>0){
    	    	_refloc=ref.index[_seed].loc1;  //list of seeded reference locations
    	    	h=param.profile[modeindex][0].a+cseed_offset+seed_start_offset;
    	    	for(j=0; j!=m; j++) {
    	    		_hit=_refloc[j];
    	...
    	modeindex=seedindex[mode].second;
        	for(i=0; i!=param.index_interval; i++) {
        		...
        		//cout<<"mode:"<<mode<<" i= "<<i<<" m="<<ref.index2[_seed][0]<<" seed:"<<_seed<<endl;
                if(ref.index2[_seed]==NULL) continue; //no match
        		m=ref.index2[_seed][0];  mc=ref.index2[_seed][1];
        		//cout<<"mode:"<<mode<<" i= "<<i<<" m="<<m<<" mc="<<mc<<" seed:"<<_seed<<endl;
        		h=-param.profile[modeindex][i].a+i-seed_start_offset;
...
void SingleAlign::ReorderSeed(RefSeq &ref) {
    bit32_t i,ii,s,cs, tt, total=0xffffffff;
    if(param.RRBS_flag) seed_start_offset=0;
    else {
    ...
    seedindex.clear(); cseedindex.clear();
    //cout<<"seedseg_num:"<<seedseg_num<<endl;
    for(i=0; i<seedseg_num; i++){
        GenerateSeeds(i, seed_start_offset);  
        s=0;cs=0;
        //cout<<"i="<<i<<endl;
        if(param.RRBS_flag){
            s+=ref.index[seeds[i][0]].n1;
            cs+=ref.index[cseeds[i][0]].n1;
        }
        else {
            for(ii=0; ii!=param.index_interval; ii++){
                //cout<<"ii="<<ii<<" n="<<ref.index2[seeds[i][ii]][0]<<endl;
                if(ref.index2[seeds[i][ii]]!=NULL) s+=ref.index2[seeds[i][ii]][0];
                if(ref.index2[cseeds[i][ii]]!=NULL) cs+=ref.index2[cseeds[i][ii]][0];
            }
        }
        seedindex.push_back(pair<int,int>(s,i)); cseedindex.push_back(pair<int,int>(cs,i));
    }
...
bit32_t SingleAlign::GetTotalSeedLoc(RefSeq &ref, int start) {
    int i,ii, total=0;
    bool flag=param.chains||(_pread->readset<2), cflag=param.chains||(_pread->readset==2);
    for(i=0; i<seedseg_num; i++){
        GenerateSeeds(i, start);
        for(ii=0; ii!=param.index_interval; ii++){
            if(flag)
                if(ref.index2[seeds[i][ii]]!=NULL) total+=ref.index2[seeds[i][ii]][0];
            if(cflag)
                if(ref.index2[cseeds[i][ii]]!=NULL) total+=ref.index2[cseeds[i][ii]][0];
        }
   }
   return total;
}
